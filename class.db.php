...
class db extends PDO {
	...
	private function debug() {
		if(!empty($this->errorCallbackFunction)) {
			...
			if($this->errorMsgFormat == "html") {
				...
				$css = trim(file_get_contents(dirname(__FILE__) . "/css/bootstrap.css"));
				$msg .= '<style type="text/css">' . "\n" . $css . "\n</style>";
				$msg .= "\n" . '<div class="alert alert-danger">' . "\n\t<h3>SQL Error</h3>";
				foreach($error as $key => $val)
					$msg .= "\n\t<label>" . $key . ":</label><pre>" . $val . "</pre>";
				$msg .= "\n\t</div>\n</div>";
			}
	...
	public function run($sql, $bind="", $return_object=false) {
		$this->sql = trim($sql);
        ...
        if($return_object)
          return $pdostmt->rowCount() == 1 ? $pdostmt->fetchObject() : false;
